...
import {CityStats} from "$lib/entities/models";
...
// GetTopCities returns a list of cities with their booking stats, useful for reporting
export function GetTopCitiesByCountOfTicketsBooked(bookings: Booking[]): CityStats[] {
    const cities: CityStats[] = bookings.reduce((accum: CityStats[], booking: Booking) => {
        ...
        // account for malformed city names we've seen in the db
        if (cityName === 'belrin') {
            cityName = 'berlin'
        }
        ...
        // get the cityIndex to be updated. if entry doesn't exist, create it
        ...
        switch (booking.payment_status){
            case values.BookingPaymentStatus.PAID:
                accum[cityIndex].totalPaidBookings += ticketsBooked
                break
            case values.BookingPaymentStatus.UNPAID:
    ...
    const totalBookingsCount: number = cities.reduce((accum, city) => accum + city.totalBookings, 0)

    const citiesWithPercentages: CityStats[] = cities.map((city) => {
        // calculate percentage of paid bookings
        if (city.totalBookings > 0) {
            city.percentOfPaidBookings = (city.totalPaidBookings / city.totalBookings)
        }

        // calculate how much is this city's bookings contribution compared to all bookings
        city.percentOfThisCitysBookingsOverAllBookings = (city.totalBookings / totalBookingsCount)
        return city
    })

    const topCitiesByBookingsCount = citiesWithPercentages.sort((a, b) => b.totalBookings - a.totalBookings)

    return topCitiesByBookingsCount
}
